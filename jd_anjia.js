...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityUrl = `https://lzkjdz-isv.isvjcloud.com/pool/captain/${$.authorNum}?activityId=${$.activityId}&signUuid=&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await anjia();
            await $.wait(4000)
            if ($.bean > 0) {
...
async function anjia() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            await $.wait(2000)
            await task('activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&signUuid=`)
            if ($.activityContent) {
                if ($.activityContent.canJoin) {
                    $.log("加入队伍成功，请等待队长瓜分京豆")
                    await $.wait(2000)
                    await task('saveCandidate', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&signUuid=&pinImg=${encodeURIComponent(`https://img10.360buyimg.com/imgzone/jfs/t1/21383/2/6633/3879/5c5138d8E0967ccf2/91da57c5e2166005.jpg`)}`)
                    $.log("加入会员")
                    ...
                    await $.wait(2000)
                    await task('activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&signUuid=`, 0, 1)
                    await $.wait(2000)
                    if ($.index === 1) {
