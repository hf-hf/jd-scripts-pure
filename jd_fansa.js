...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/shop/league/activity?activityId=${$.activityId}&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
...
async function superFans() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            await task('shop/league/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${encodeURIComponent($.pinImg)}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=`)
            if ($.activityContent) {
                await task('shop/league/checkOpenCard', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=${$.actorUuid}`)
                $.log("\n关注店铺")
                if (!$.activityContent['followShop'].allStatus) {
                    await task('shop/league/saveTask', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=${$.actorUuid}&&taskType=1&taskValue=1`)
                } else {
                ...
                if (!$.activityContent['addSku'].allStatus) {
                    await task('shop/league/saveTask', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=${$.actorUuid}&taskType=2&taskValue=2`)
                } else {
                ...
                if ($.openCardStatus) {
                    ...
                    for (const vo of taskList) {
                        ...
                        await task(
                            ...
                            `activityId=${$.activityId}&actorUuid=${$.actorUuid}&pin=${encodeURIComponent($.secretPin)}`
                        );
                        await $.wait(2000);
                    }
                } else {
