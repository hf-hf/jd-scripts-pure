...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityUrl = `https://lzdz1-isv.isvjcloud.com/dingzhi/vivo/iqoojieyapa/activity/${$.authorNum}?activityId=${$.activityId}&shareUuid=''&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await vivo();
            await $.wait(3000);
            if ($.bean > 0) {
...
async function vivo() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            await $.wait(2000);
            await task('vivo/iqoojieyapa/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${encodeURIComponent($.pinImg)}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&shareUuid=`)
            await $.wait(2000);
            ...
            if ($.activityContent) {
                ...
                if (!$.activityContent.followShopStatus) {
                    await task('vivo/iqoojieyapa/saveTask', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=${encodeURIComponent($.actorUuid)}&taskType=23&taskValue=&shareUuid=`)
                } else {
                    $.log("    >>>已经关注过了\n")
                }
                $.log("\n-> 任务")
                await $.wait(2000);
                await task('vivo/iqoojieyapa/saveTask', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=&shareUuid=&taskType=0&taskValue=`)
                await $.wait(2000);
                await task('vivo/iqoojieyapa/saveTask', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=&shareUuid=&taskType=12&taskValue=1`)
                await $.wait(2000);
                await task('vivo/iqoojieyapa/saveTask', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=&shareUuid=&taskType=21&taskValue=`)
                await $.wait(2000);
                await task('vivo/iqoojieyapa/saveTask', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&actorUuid=&shareUuid=&taskType=10&taskValue=10`)
                await $.wait(2000);
