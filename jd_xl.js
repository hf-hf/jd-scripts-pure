...
!(async () => {
    ...
    for (let i = 0; i < cookiesArr.length; i++) {
        if (cookiesArr[i]) {
            ...
            $.activityUrl = `https://lzdz4-isv.isvjcloud.com/dingzhi/xiaolong/collectcard/activity?activityId=${$.activityId}&adsource=null&shareuserid4minipg=null&shopid=${$.activityShopId}&lng=00.000000&lat=00.000000&sid=&un_area=`
            await xl();
        }
    }
...
async function xl() {
    ...
    if ($.token) {
        ...
        if ($.secretPin) {
            ...
            await task('xiaolong/collectcard/activityContent', `activityId=${$.activityId}&pin=${encodeURIComponent($.secretPin)}&pinImg=${encodeURIComponent($.pinImg)}&nick=${encodeURIComponent($.pin)}&cjyxPin=&cjhyPin=&picId=`)
            if ($.activityContent) {
